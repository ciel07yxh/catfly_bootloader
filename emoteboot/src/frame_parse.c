...
void input(struct zxy_framer *phaser,uint8_t *buf,uint8_t len)
{
    ...
    switch(frame->type)
    {
        case APP2BOOT:
					...
					while(BOOT_ErasePage(APP2BOOT_ADDR,APP_EXE_FLAG_START_ADDR+0x10000)!=FLASH_COMPLETE);
					is_flash=1;
					FLASH_Lock();	
					__set_PRIMASK(0);
        //ack frame callback
        ...
        break;
				...
				//	{
					...
					if(sizeof(frame_moteid_t)==frame->payload_len)
					{
						...
						//TODO write modeid
						moteid = moteid_frame->moteid << 16;
						FLASH_Unlock();
						while(BOOT_ProgramDatatoFlash(MOIEID_ADDR,(uint8_t *)&moteid,4)!=FLASH_COMPLETE){
